...
#####    	设置为网格布局之后，项目的float, display:inline-block, display: table-cell, vertical-align 和column-*等设置都失效。

### 2.2 grid-template-columns属性和grid-template-rows属性
...
##### 2.2.2 auto-fill 关键字  自动填充

```
.container {
	display: grid;
	grid-template-columns: repeat(auto-dill, 100px);
}
```

##### 2.2.3 fr关键字  意为片段

##### 2.2.4 minmax() 函数 产生一个长度范围，接受两个参数，分别为最小值和最大值

##### 2.2.5 auto 关键字 表示由浏览器自己决定长度。

##### 2.2.6网格线名称

​	使用方括号指定每一根网格线的名字，还允许一根线有多个名字。
